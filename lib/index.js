...
module.exports = function gulpMultijnect(files, name, options) {
  ...
  return through.obj(function(file, enc, cb) {
    var self = this;
    if (file.isStream()) {
      this.emit('error', new PluginError('gulp-multinject', 'Streaming not supported'));
      return cb();
    }
        ...
        function(match, startTag, indentation, ignore, endTag) {
          return [startTag]
            .concat(_.map(files, function(injFile) {
              ...
              if(!template) {
                self.emit('error', new PluginError('gulp-multinject', "Cannot find template for rendering a " 
